...
class MongoKnowledgeBase:
    ...
    def investigate_vectors(self) -> str:
        ...
        try:
            ...
            result = f"MongoDB Atlas Vector store for user {self.user_id}:\n"
            result += f"- Collection: {self.collection_name}\n"
            result += f"- Total documents: {total_docs:,}\n"
            result += f"- Embedding dimensions: {dimensions:,}\n"
            result += f"- Vector index: {self.index_name}\n"
            result += f"- Index status: {vector_index_status}\n"
            
            if doc_types:
                doc_type_str = ', '.join(f'{dt["_id"]}({dt["count"]})' for dt in doc_types if dt["_id"])
                result += f"- Document types: {doc_type_str}"
